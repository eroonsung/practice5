...
namespace Client
{
    public partial class AIPlayForm : Form
    {
        ...
        private int limit = 1; // 내다보고자 하는 수를 의미
        ...
        */
        ...
        private int evaluate(Horse horse) // 매개변수로 특정한 플레이어가 들어옴(인공지능 혹은 사용자)
        {
            ...
            for (int i = 0; i < edgeCount; i++)
            {
                for (int j = 0; j < edgeCount; j++)
                {
                    ...
                    if (board[i, j] == horse)
                    {
                        ...
                        {
                            return 1000000; // 가장 큰 가치를 가지는 값을 반환
                        }

                        /********** 단순히 전체에서 4목이 만들어진 경우만 체크(16가지 경우) **********/

                        count = make4mok1(i, j);
                        //왼쪽과 오른쪽이 모두 비어있다면 open4를 증가시켜 열린 4라는 것을 알림
                        if (count == 2) open4++;
                        ...
                        /********** 단순히 전체에서 3목이 만들어진 경우만 체크 (12가지 경우)**********/
                        ...
                        /********** 단순히 전체에서 2목이 만들어진 경우만 체크 (4가지 경우)**********/
            ...
            //alpha : max의 역할
        {
            ...
            // MAX 부분에 해당하는 경우(인공지능의 입장)
            if (level % 2 == 0)
            {
                // 더이상 작아질 수 없는 수를 max로 설정
                int max = -1000000;
                // 탐색을 끝내는 경우 find를 1로 설정, 탐색이 끝나지 않았다면 0
                int find = 0;
                ...
                for (int i = 0; i < edgeCount; i++)
                {
                    for (int j = 0; j < edgeCount; j++)
                    {
                        ...
                        if (board[i, j] == 0)
                        {
                            ...
                            board[i, j] = 0;
                            // 만약에 더욱 효율적인 수를 찾은 경우(가장 큰 가치를 고르는 경우)
                            if (max < e)
                            {
                                // 그 수로 타겟을 설정
                                max = e;
            ...
            // MIN 부분에 해당하는 경우(player의 입장, 사용자)
            else
            {
